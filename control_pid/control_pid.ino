...
const float Ki = 0.5f;      // Ganancia integral
const float Kd = 0.03f;      // Ganancia derivativa
...
void loop() {
  ...
  // ----- 3. Integrador (con anti-windup) -----
  I_term += Ki * Ts_s * error_deg;
  if (I_term > I_MAX) I_term = I_MAX;
  if (I_term < I_MIN) I_term = I_MIN;

  // ----- 4. Derivada filtrada (Tustin bilineal) -----
  float xk = theta_pend_deg;
  ydot_f = c0 * xk + c1 * x_prev - d1 * ydot_f;
  x_prev = xk;

  // ----- 5. Control PID -----
  float u_deg = -Kp * error_deg - I_term - Kd * ydot_f;

  // ----- 6. Convertir u a posición del servo -----
  ...
  // ----- 7. Enviar tiempo y salida -----
  Serial.print(t_s, 3);
  Serial.print(",");
  Serial.println(theta_pend_deg, 3);

  // ----- 8. Mantener 20 ms de muestreo -----
